...
from modeling.backbone.conv2_5d import Conv2_5d
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, BatchNorm=None):
        downsample = None
        #有下采样stride=2，或者planes需要更新，初始self.inplane=64
        ...
        layers = []
        #layer的第一个module，可能包含下采样（stride=2），后面的module都不包含下采样
        layers.append(BottleneckDepth(self.inplanes, planes, stride, dilation, downsample, BatchNorm))
        self.inplanes = planes * block.expansion
        #生成(blocks-1)个连续module，与第一个module一起组成一个layer
    ...
    def _make_MG_unit(self, block, planes, blocks, stride=1, dilation=1, BatchNorm=None):
        ...
        layers = []
        layers.append(BottleneckDepth(self.inplanes, planes, stride, dilation=blocks[0]*dilation,
...
if __name__ == "__main__":
    import torch
    model = ResNet101(BatchNorm=nn.BatchNorm2d, pretrained=False, output_stride=16)
    input = torch.rand(3, 3, 512, 512)
    depth = torch.rand(3, 1, 512, 512)
    f = 1
